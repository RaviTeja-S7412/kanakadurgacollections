...
class Home extends CI_Controller
{
   ...
   function vendor_logup($para1 = "", $para2 = "")
    ...
    function cart_finish($para1 = "", $para2 = "")
    {
      ...
      else if ($this->input->post('payment_type') == 'phonepay') {
            if ($para1 == 'go') {
                $data['product_details']   = $product_details;
                $data['shipping_address']  = json_encode($_POST);
                $data['vat']               = $vat;
                ...
                $details = json_decode($data['shipping_address']);
                $order_id = uniqid(); 
                $name=$details->firstname.' '.$details->lastname;
                $email=$details->email;
                $mobile=$details->phone;
                $amount = $data['grand_total']; // amount in INR
                $description = 'Payment for Product/Service';
                

                $apiKey = '099eb0cd-02cf-4e2a-8aca-3e6c6aff0399';
                $merchantId = 'PGTESTPAYUAT';
                // $apiKey = '75cbb884-0044-43b3-b3fd-036185634e51';
                // $merchantId = 'KANAKADURGAONLINE';
                $url = 'https://www.phonepe.com/api/v1/payment/initiate';
                $redirectUrl = base_url().'home/phone_pay_success';
                
                $paymentData =[
                    'merchantId' => $merchantId,
                    'merchantTransactionId' => 'KDC'.date('Ymdhis',time()), // test transactionID
                    "merchantUserId"=>$data['buyer'],
                    'amount'=> $amount*100,
